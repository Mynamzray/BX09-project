...
// 2. CSS 樣式表區（原生 CSS，完美還原暗黑科技感，零外部依賴）
...
// 4. JAVASCRIPT 區塊（核心渲染與通訊邏輯，支援動態狀態解碼）
const char WEB_JS[] PROGMEM = R"rawliteral(
    <script>
        ...
        function drawChart(tArr, rawArr, filteredArr) {
            ...
            const maxRPM = Math.max(...filteredArr, ...rawArr, 5000);
        ...
        function initWebSocket() {
            ...
            socket.onopen = () => {
                ...
                document.getElementById('statusText').innerText = '與戰情室連線中...';
        ...
        function handleSync(data) {
            if (data.peak > 0) {
                ...
                document.getElementById('durationVal').innerHTML = `${data.duration}<span style="font-size: 1rem; font-weight: normal;">ms</span>`;
            }
            handleStatus(data); 
        }
